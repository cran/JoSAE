...
reliable estimate. Frequently, auxiliary variables exists that are correlated
...
\begin{itemize}
   ...
   \item domain data which contain the mean of the auxiliary variables
     of all population elements within each domain of interest.
\end{itemize}
...
canopy height derived from overlapping digital aerial images within
...
\citet{battese1988} the EBLUP estimator and demonstrated its
...
\citet{schoch2011} the Landsat data are available in R. The functions
...
estimation where outliers do not need to be excluded.} in row 33 as
...
\bibitem[Breidenbach and Astrup (2011)]{bre2011} Breidenbach, J. and
  R. Astrup (2011): \emph{Small area estimation of forest attributes
    in the Norwegian National Forest Inventory}. European Journal of
  Forest Research, under review.
%
